...
def visualize_horizontal_displacement(rgb_path, csv_path, displacement_csv_path, results_path, base_name):
    """
    Visualize cracks in the DEM and display the calculated vertical displacement for each crack.
    - Show cracks as outlined regions.
    - Display vertical displacement values for each crack.
...
def visualize_looping(dem_folder, csv_folder, displacement_folder, results_folder, mode="vertical"):
    """Loops through all displacement CSV files and visualizes displacement."""
    ...
    for disp_filename in sorted(os.listdir(displacement_folder)):
        if disp_filename.endswith("VERT_DISP.csv"):  # Adjust file extension if needed
            ...
            base_name = disp_filename.replace("VERT_DISP.csv", "")
            dem_path = os.path.join(dem_folder, base_name + "RGB.jpg")
            csv_path = os.path.join(csv_folder, base_name + "MASK.csv")
            if mode == "horizontal":
                visualize_horizontal_displacement(dem_path, csv_path, displacement_csv_path, results_folder, base_name)
            else:
